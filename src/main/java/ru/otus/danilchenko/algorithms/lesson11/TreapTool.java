...
public class TreapTool<T> {
    private IComparator<T> comparator;
    private Node internalTree;

    public TreapTool(IComparator<T> comparator) {
    ...
    public T optimalTreeSearch(T value) {
        final var node = internalTree.search(value);
        if (node == null) {
            return null;
        }
        if (node != internalTree) {
            remove(node.key);
            int power = node.power + 1;
            if (power < Integer.MAX_VALUE) {
                power += 1;
            } else {
                //todo recalculate power of tree
            }
            insert(node.key, power);
        }
        return node.key;
    }


    public void insert(T value, int power) {
        Objects.requireNonNull(value);
        var node = new Node(value, power, null, null);
